...
class _AddRequirementScreenState extends State<AddRequirementScreen> {
  ...
  List<Widget> _buildChemicalRequirementFields() {
    final palette = context.labmate;
    final colorScheme = context.colorScheme;
  ...
  List<Widget> _buildConsumableRequirementFields() {
    final palette = context.labmate;
    final colorScheme = context.colorScheme;
    ...
    return [
      ...
      if (isOtherConsumableCategory) ...[
        TextFormField(
          ...
          style: TextStyle(color: colorScheme.onSurface),
          decoration: inputDecoration('Consumable Item Name / Variant'),
          onChanged: (_) {
            setState(() {
              _refreshConsumableTypePreview();
            });
  ...
  List<Widget> _buildRequirementDetailsFields() {
    return [
      _buildRequirementTypeDropdown(),
      const SizedBox(height: 14),
      if (selectedMainType == 'chemical') ..._buildChemicalRequirementFields(),
      if (selectedMainType == 'consumable')
        ..._buildConsumableRequirementFields(),
    ];
  }

  List<Widget> _buildPurchaseDetailsFields() {
    final colorScheme = context.colorScheme;
  ...
  Widget _buildCalculatedSummaryCard(double total) {
    final palette = context.labmate;
    final colorScheme = context.colorScheme;
  ...
  List<Widget> _buildBottomFields(double total) {
    return [
      _buildCalculatedSummaryCard(total),
      const SizedBox(height: 22),
      _buildSubmitButton(),
    ];
  }

  Widget _buildNarrowFormContent(double total) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        ..._buildRequirementDetailsFields(),
        ..._buildPurchaseDetailsFields(),
        const SizedBox(height: 18),
        ..._buildBottomFields(total),
      ],
    );
  }

  Widget _buildWideFormContent(double total) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Row(
          ...
          children: [
            Expanded(
              child: _sectionCard(
                ...
                children: _buildRequirementDetailsFields(),
              ),
            ),
            ...
            Expanded(
              child: _sectionCard(
                ...
                children: _buildPurchaseDetailsFields(),
              ),
            ),
          ],
        ),
                ...
                children: [
                  Center(
                    child: ConstrainedBox(
                      ...
                      child: isWide
                          ? _buildWideFormContent(total)
                          : _buildNarrowFormContent(total),
                    ),
                  ),
                ],
              );
